...
echo "${log}" | grep -q "DOCKER pull ghcr.io/sempersupra/winebotappbuilder-winbuild:stable" || { echo "Missing build pull-first action" >&2; exit 1; }
echo "${log}" | grep -q "DOCKER pull ghcr.io/sempersupra/winebotappbuilder-packager:stable" || { echo "Missing package pull-first action" >&2; exit 1; }
echo "${log}" | grep -q "DOCKER pull ghcr.io/sempersupra/winebotappbuilder-signer:stable" || { echo "Missing sign pull-first action" >&2; exit 1; }
echo "${log}" | grep -q "DOCKER run .*ghcr.io/sempersupra/winebotappbuilder-winbuild:stable" || { echo "Missing build container run action" >&2; exit 1; }
echo "${log}" | grep -q "DOCKER run .*ghcr.io/sempersupra/winebotappbuilder-packager:stable" || { echo "Missing package container run action" >&2; exit 1; }
echo "${log}" | grep -q "DOCKER run .*ghcr.io/sempersupra/winebotappbuilder-signer:stable" || { echo "Missing sign container run action" >&2; exit 1; }
