...
echo "${log}" | grep -q "DOCKER pull ghcr.io/sempersupra/winebotappbuilder-winbuild:v0.2.0" || { echo "Missing build pull-first action" >&2; exit 1; }
echo "${log}" | grep -q "DOCKER pull ghcr.io/sempersupra/winebotappbuilder-packager:v0.2.0" || { echo "Missing package pull-first action" >&2; exit 1; }
echo "${log}" | grep -q "DOCKER pull ghcr.io/sempersupra/winebotappbuilder-signer:v0.2.0" || { echo "Missing sign pull-first action" >&2; exit 1; }
echo "${log}" | grep -q "DOCKER run .*ghcr.io/sempersupra/winebotappbuilder-winbuild:v0.2.0" || { echo "Missing build container run action" >&2; exit 1; }
echo "${log}" | grep -q "DOCKER run .*ghcr.io/sempersupra/winebotappbuilder-packager:v0.2.0" || { echo "Missing package container run action" >&2; exit 1; }
echo "${log}" | grep -q "DOCKER run .*ghcr.io/sempersupra/winebotappbuilder-signer:v0.2.0" || { echo "Missing sign container run action" >&2; exit 1; }
