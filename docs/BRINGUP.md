...
- append-only audit stream for command/operation traceability (`agent-sandbox/state/audit-log.sqlite`)
...
- daemon preflight diagnostics summary (`agent-sandbox/state/preflight-status.json`, `preflight_status`)
- daemon preflight trend counters (`agent-sandbox/state/preflight-counters.json`, `command.preflight` counters)
